 ...
 */
public class Part1 extends ListGraph {
     ...
     */
    public Part1(int numV, boolean directed){
        super(numV,directed);
    }
    ...
    public void plot_graph(Graph graph){
        for(int i = 0; i<graph.getNumV(); i++){
            ...
            for(int k = 0; k<graph.getNumV(); k++){
                if(graph.isEdge(i,k)){
                    System.out.print("-(" + graph.getEdge(i,k).getWeight() + ")->" + graph.getEdge(i,k).getDest());
                }
            }
            System.out.print("\n");
        }
          ...
          /* Insert the edge (current, neighbor)
             into the tree. */
                    ...
                    for(int i = 0 ; i<getNumV(); i++){
                        if(isEdge(current, i) && !identified[i]){
