...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    private boolean insert(Node<E> root, E item) {
        int index = binarySearch(item, root, 0, root.size);
    ...
    private int binarySearch(E item, Node<E> localRoot, int i, int size) {
        int middle = (i + size) / 2;
        if(size < i){
            return middle+1;
        }
        ...
        if(item.compareTo(localRoot.data[middle])==0) {
            return middle;
        }
        else if(item.compareTo(localRoot.data[middle])>0) {
            return binarySearch(item, localRoot, middle + 1, size);
        }
        else if(item.compareTo(localRoot.data[middle])<0) {
            return binarySearch(item, localRoot, i, middle-1);
        }
        if(size == middle && i == middle){
            if(item.compareTo(localRoot.data[middle])>0){
                return binarySearch(item, localRoot.child[middle+1], i, root.size);
            }
            if(item.compareTo(localRoot.data[middle])<0){
                return binarySearch(item, localRoot.child[middle], i, root.size);
            }
        }
        return middle;
    }

    public void BinarySearchTest(E item){
        System.out.print(binarySearch(item,root, 0, root.size));
    }
    /*</listing>*/
