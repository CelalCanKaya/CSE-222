...
public class AVLTree < E
    ...
    public AVLTree(BinaryTree < E > checkTree) {
        boolean status = isBST(checkTree.root, checkTree.root.data, checkTree.root.data);
        if(status==false){
            System.out.print("Bu Tree Bir AVLTree Değil!\n");
        }
        else {
            status = isBalanced(root);
            if (status == false) {
                System.out.print("Bu Tree Bir AVLTree Değil!\n");
            } else {
                System.out.print("Bu Tree Bir AVLTree'dir.");
            }
        }
    }
    ...
    private boolean isBST(Node node, E min, E max){
        ...
        else if (min.compareTo((E) node.data)>0 || max.compareTo((E) node.data)<0){
            return false;
        }
        return (isBST(node.left, min, (E) node.data) && isBST(node.right, (E) node.data, max));
    }
